...
- **Date**: 2026-02-23
- **Tester**: Claude Code (Playwright MCP 브라우저 자동화)
- **Target URL**: http://localhost:5173 (Vite 개발서버) / http://localhost:8080 (Spring Boot)
- **Tooling**: Playwright MCP (browser_snapshot/evaluate/navigate/click 등), API 직접 호출 (fetch), 네트워크 인터셉트 (page.route)
...
| TC-01-004 | Form | ExamLogin 중복 클릭 방지 | 활성 시험 존재 | 1) /exam/login 접속 2) 이름+생년월일 입력 3) 시험 시작 클릭 4) 로딩 중 재클릭 시도 | 로딩 중 버튼 disabled, 중복 요청 차단 | Pass (코드 검증) | Important #9. API 응답이 빨라 런타임 관찰 불가. :disabled="loginLoading" 바인딩 + handleLogin 내 `if (loginLoading.value) return` 가드를 코드 수준에서 확인 |
