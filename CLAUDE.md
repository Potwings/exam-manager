...
│       ├── views/
│       │   ├── admin/       # AdminLogin, ExamManage, ExamCreate, ExamDetail, ExamMonitor, ScoreBoard, ScoreDetail, AdminMembers, ChangePassword
│       │   └── exam/        # ExamLogin, ExamTake
│       └── router/          # Vue Router
├── backend/                 # Spring Boot
│   └── src/main/java/com/exammanager/
│       ├── config/          # SecurityConfig, WebConfig, OllamaProperties, OpenAiProperties, LlmProperties, LlmClientConfig, CorsProperties, AdminInitializer, InitLoginFilter, ExamTimeUtils
│       ├── controller/      # AdminController, ExamController, ExamineeController, SubmissionController, ScoreController, AiAssistController, ExamSessionController, NotificationController, MonitorController
│       ├── service/         # ExamService, ExamineeService, DocxParserService, GradingService, LlmClient, OllamaClient, OpenAiClient, SubmissionService, AiAssistService, AdminUserDetailsService, NotificationService, MonitorService
...
```
- `LlmClient.java` — 인터페이스 (`isAvailable()`, `chat(systemPrompt, userPrompt) → JsonNode`, `chat(List<ChatMessage> messages) → JsonNode` 멀티턴)
...
- 독립: 기존 Card 유지
- **채점 기준 표시**: `answerContent` 필드로 문제별 채점 기준(Answer.content) 표시
  - 읽기 모드: 채점 사유 아래에 Collapsible 접이식 (기본 접힘, 클릭 시 펼침)
  - 편집 모드: EditForm 상단에 blue info 스타일로 항상 펼쳐서 표시
  - `answerContent`가 null인 문제는 채점 기준 섹션 미표시
...
- Props: `open`, `problem`, `examId`, `isGroupParent`, `parentProblemNumber`, `parentContent`
...
- **AI 출제 도우미**: "문제 내용" Label 우측에 Sparkles 버튼 배치 (`!isGroupParent && aiAvailable`), AiAssistDialog 재사용하여 문제/채점기준/배점/콘텐츠타입 AI 생성·반영. 그룹 자식은 `parentContent` prop으로 부모 지문을 AiAssistDialog에 전달
- `watch(open)` — Dialog 열릴 때 problem 데이터를 form으로 복사, 미리보기/에러 상태 초기화, AI 가용 상태 최초 1회 확인

#### ExamDetail.vue 연동
- 각 문제 카드 헤더에 SquarePen 편집 아이콘 버튼 (독립/그룹 부모/그룹 자식 3곳, `aria-label`로 문제 번호 포함한 접근성 레이블 제공)
- `openEditDialog(problem, isGroupParent, parentProblemNumber, parentContent)` — 편집 대상 설정 + Dialog 열기 (그룹 자식은 부모 content 전달)
...
- `POST /api/exam-sessions` — find-or-create 패턴 (DataIntegrityViolationException 처리), 시간 제한 유무와 관계없이 항상 세션 생성
  - timeLimit 없으면 `{ remainingSeconds: null }` 반환
...
| Submission | submissions | 제출 답안 (submittedAnswer, isCorrect, earnedScore, **feedback**, **annotatedAnswer**, **regrading**) → Examinee, Problem N:1 |
...
| POST | `/api/submissions/{id}/regrade` | SubmissionController | 개별 submission 재채점 (비동기 LLM) — **Admin** |
| POST | `/api/submissions/regrade` | SubmissionController | 수험자+시험 단위 전체 재채점 (비동기 LLM) — **Admin** |
...
| GET | `/api/monitor/sessions` | MonitorController | 시험별 응시 현황 조회 (세션 목록 + 남은 시간) — **Admin** |
...
| ChatMessage | LLM 멀티턴 대화 메시지 (role, content) — LlmClient·AiAssistRequest 공용 |
| AiAssistRequest | AI 출제 요청 (@NotBlank instruction, parentContent, contentType, score, **conversationHistory**: List\<ChatMessage\>) |
...
| RegradeRequest | 전체 재채점 요청 (examineeId, examId) |
| SubmissionResultResponse | 채점 결과 응답 (totalScore, maxScore, submissions[{..., **feedback**, **codeAnswer**, **codeLanguage**, **parentProblemId**, **parentProblemNumber**, **parentProblemContent**, **parentProblemContentType**, **answerContent**}]) |
...
| ExamSessionMonitorResponse | 응시 현황 응답 (examineeId, examineeName, examineeBirthDate, status, remainingSeconds, startedAt) — status: IN_PROGRESS/SUBMITTED/TIME_EXPIRED |
...
| `/admin/monitor` | ExamMonitor | 응시 현황 모니터링 — **Admin 가드** |
...
- `ExamineeService.loginOrCreate()` — 이름+생년월일로 기존 수험자 조회 → 없으면 신규 생성 (find-or-create)
- `ExamineeController` → `ExamineeService` → `ExamineeRepository` 계층 구조
- 동일 수험자의 재시험 방지에 활용 (같은 examineeId로 이미 제출 기록 존재 시 409)

### 재시험 방지 (이중 방어)
- **로그인 시점 (1차)**: `ExamineeService.loginOrCreate()`에서 기존 수험자인 경우, `SubmissionRepository.existsByExamineeIdAndActiveExam()` (@Query JPQL)로 활성 시험에 대한 제출 기록 확인 → 있으면 `409 CONFLICT`. 신규 수험자는 검증 스킵
- **제출 시점 (2차)**: `SubmissionService.submitAnswers()` 시작 시 `existsByExamineeIdAndProblemExamId()` 검증 → 이미 제출 기록이 있으면 `409 CONFLICT`
- 프론트엔드에서 409 응답 시 인라인 에러 메시지 표시 ("이미 응시 완료한 시험입니다."), 입력 변경 시 자동 초기화
...
### 응시 현황 모니터링 (`ExamMonitor.vue`)
관리자가 현재 시험 응시 상태를 실시간으로 모니터링하는 페이지.

#### 아키텍처
```text
GET /api/monitor/sessions?examId={examId}  ← Admin 인증 필수
  → MonitorService.getSessionsByExam()
    → ExamSessionRepository.findByExamIdWithExaminee() (fetch join)
    → 각 세션별 제출 여부 확인 (SubmissionRepository)
    → 상태 판별 + 남은 시간 계산 (ExamTimeUtils)
    → ExamSessionMonitorResponse 리스트 반환 (startedAt 역순)
```

#### 상태 판별 로직 (`MonitorService`)

| 조건 | status | remainingSeconds |
|------|--------|-----------------|
| 제출 완료 | `SUBMITTED` | null |
| 시간 제한 있고 시간 초과 | `TIME_EXPIRED` | 0 |
| 시간 제한 있고 시간 남음 | `IN_PROGRESS` | 양수 |
| 시간 제한 없고 미제출 | `IN_PROGRESS` | null |

#### ExamSession 항상 생성
- `ExamSessionController.createSession()` — 시간 제한 유무와 관계없이 항상 ExamSession 생성 (find-or-create)
- 기존: timeLimit 없으면 세션 미생성 → 모니터링 불가
- 변경: 세션 항상 생성, timeLimit 없으면 `remainingSeconds: null` 응답 (기존 응답 동일)

#### 프론트엔드 (`ExamMonitor.vue`)
- **시험 선택**: 드롭다운으로 시험 선택, 활성(active) 시험 자동 선택
- **테이블**: 이름, 생년월일, 상태 Badge, 남은 시간(MM:SS), 시작 시각
- **상태 Badge**: 응시 중(default) / 제출 완료(secondary) / 시간 만료(destructive)
- **카운트다운**: 클라이언트 1초 간격 + 서버 10초 폴링으로 drift 보정
- **색상 변화**: 5분 이하 amber, 1분 이하 red + pulse
- **요약**: "총 N명 · 응시 중 N명 · 제출 완료 N명"

#### 시간 계산 유틸 (`ExamTimeUtils`)
- `config` 패키지의 순수 유틸 클래스 (인스턴스화 불가)
- `calculateRemainingSeconds(startedAt, timeLimitMinutes)` — 남은 초 계산, 0 미만 보정
- `MonitorService`, `ExamSessionController` 양쪽에서 공용 사용

#### 헤더 네비게이션
- 탭 순서: 채점결과 | **응시현황** | 시험관리 | 계정관리
...
### 멀티턴 대화 구조
이전 대화 히스토리를 LLM Chat API의 messages 배열에 user/assistant 역할로 전달하는 멀티턴 방식:
```
system: "출제 전문가..."
assistant: {"problemContent":"기존 문제",...}     ← 초기 콘텐츠 (있는 경우)
user: "Java 상속 문제 만들어줘"                    ← 1차 요청
assistant: {"problemContent":"결과",...}           ← 1차 응답
user: "더 어렵게"                                  ← 2차 요청 (현재 instruction)
```

#### 데이터 흐름
- **프론트엔드**: `AiAssistDialog.vue`의 `history` 배열 전체를 `conversationHistory` 필드로 변환하여 서버 전송
  - Dialog 열릴 때 기존 문제가 있으면 `initialContent` ref에 저장 → 첫 요청 시 assistant 메시지로 포함
  - `history.value.slice(0, -1)`로 현재 입력 제외 (현재 입력은 `instruction` 필드로 별도 전송)
- **백엔드**: `AiAssistService.buildMessages()`에서 system + conversationHistory + 현재 user 메시지 조립
  - **보안**: `ALLOWED_ROLES = Set.of("user", "assistant")`로 system 역할 주입 차단 + content null/blank 필터링
- **LlmClient**: `chat(List<ChatMessage> messages)` 멀티턴 메서드 추가 (기존 `chat(String, String)` 유지 — GradingService 하위 호환)
- **ChatMessage DTO**: `role` + `content` 필드, LlmClient와 AiAssistRequest에서 공용 사용
...
- [x] AI 채점 결과 재채점 — 개별/전체 submission 비동기 LLM 재채점 (regrading 플래그, 기존 점수 유지)
- [x] AI 출제 도우미 멀티턴 대화 — LlmClient 멀티턴 chat + conversationHistory로 전체 대화 맥락 전달
- [x] 응시 현황 모니터링 — ExamMonitor 페이지 (실시간 상태 Badge + 카운트다운 + 10초 폴링, ExamSession 항상 생성)
- [x] ProblemEditDialog AI 출제 도우미 — 단독 문제 수정 시 AiAssistDialog 재사용, "문제 내용" Label 우측 Sparkles 버튼, 그룹 자식은 부모 지문 전달
